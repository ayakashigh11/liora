...
import { ayakashi } from "#core/socket.js";
import { checkReminders } from "#lib/reminder.js";
...
async function LIORA() {
    ...
    setInterval(async () => {
        const pending = checkReminders();
        for (const r of pending) {
            try {
                await global.sock?.sendMessage(r.chat_id, {
                    text: `🔔 *REMINDER*\n\nTo: @${r.user_id.split("@")[0]}\nMessage: ${r.message}`,
                    contextInfo: { mentionedJid: [r.user_id] }
                });
            } catch (e) {
                global.logger.error({ error: e.message }, "Reminder delivery error");
            }
        }
