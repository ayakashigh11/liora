...
export async function imageToWebp(buffer, options = {}) {
    ...
    const vf = options.circle
        ? `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=fps=15,pad=320:320:-1:-1:color=0x00000000,format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='if(gt(hypot(X-W/2,Y-H/2),W/2),0,255)',split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=ffffff[p];[b][p]paletteuse`
...
export async function addExif(webpBuffer, metadata = {}) {
    ...
    const exifData = {
        ...
        "sticker-pack-publisher": metadata.packPublish || "© ayakashigh11",
