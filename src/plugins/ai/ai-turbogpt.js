/**
 * @file TurboGPT AI plugin using Nexray API
 * @module plugins/ai/turbogpt
 * @license Apache-2.0
 * @author ayakashigh11
 */
...
let handler = async (m, { sock, text, usedPrefix, command }) => {
    ...
    if (m.quoted && m.quoted.mimetype) {
        ...
        if (isTextFile) {
            try {
                ...
                sock.logger.error("Failed to read quoted file for TurboGPT:", err);
            }
        }
    }
    ...
    try {
        ...
        const apiUrl = `https://api.nexray.web.id/ai/gpt-3.5-turbo?text=${encodeURIComponent(prompt)}`;
        ...
        sock.logger.error("TurboGPT plugin error:", e);
        await m.reply(`An error occurred while contacting TurboGPT: ${e.message}`);
    } finally {
        await global.loading(m, sock, true);
    }
};

handler.help = ["turbogpt"];
handler.tags = ["ai"];
handler.command = /^(turbogpt)$/i;
